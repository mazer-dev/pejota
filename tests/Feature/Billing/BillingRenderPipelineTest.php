...
class BillingRenderPipelineTest extends TestCase
{
    ...
    {
        $this->user->company->settings()->set(CompanySettingsEnum::BILLING_EMAIL_BODY->value, '<p>Hi {{ client.name }}</p>');

        $client = Client::create(['name' => 'A & B', 'company_id' => $this->user->company->id]);
    ...
    {
        $this->user->company->settings()->set(CompanySettingsEnum::BILLING_EMAIL_SUBJECT->value, 'Invoice {{ invoice.number }} - {{ client.name }}');
